...
class profiles:
    ...
    def all(data_dump, seed):
        return randomisation_functions.badge_shuffler(
            data_dump, badge_filter("All"), seed
        )

    def action_only(data_dump, seed):
        return randomisation_functions.badge_shuffler(
            data_dump, badge_filter("Action"), seed
        )

    def bonus_only(data_dump, seed):
        return randomisation_functions.badge_shuffler(
            data_dump, badge_filter("Bonus"), seed
        )

    def expert_only(data_dump, seed):
        return randomisation_functions.badge_shuffler(
            data_dump, badge_filter("Expert"), seed
        )
    ...
    def custom(data_dump, seed):
        ...
        if len(custom_badge_selector()) > 0:
            return randomisation_functions.badge_shuffler(
                data_dump, custom_badge_selector(), seed
            )
