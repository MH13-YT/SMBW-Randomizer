...
class randomisation_scripts:
    def lite(levels_dump,seed,ignored_stages_files):
        random.seed(seed)
        for monde_num in range(1, 10):  # J'ai changé la limite à 10 pour inclure le monde 9
            ...
            if monde_a_randomiser in levels_dump:
                ...
                stage_paths_ignored = [course["StagePath"] for course in niveaux_monde if course["StagePath"] in ignored_stages_files]
                # Filtrer les stage_paths à randomiser (ceux qui ne sont pas dans la liste des ignore)
                stage_paths_randomises = [course["StagePath"]for course in niveaux_monde if course["StagePath"] not in ignored_stages_files]
                ...
                print(f"Le monde {monde_a_randomiser} n'existe pas dans votre fichier JSON et sera ignoré de la randomisation")
        return levels_dump

    def full(levels_dump,seed,ignored_stages_files):
        random.seed(seed)
