...
def get_resource_metadata(ressource):
    if not os.path.isdir(ressource['romfs']):
        return
    files_metadata = []
    ...
    for file_metadata in files_metadata:
        ...
        if file_metadata["compressed"] == True:
            if file_metadata["compression_metadata"]["compression_extension"] == ".zs":
                zstandard.zstandard_tools.decompress(f"{file_metadata['romfs_file_folder']}/{file_metadata['file_name']}{file_metadata['data_file_extension']}{file_metadata['compression_metadata']['compression_extension']}", f"{file_metadata['worktable_file_folder']}/{file_metadata['file_name']}{file_metadata['data_file_extension']}")
        else:
        ...
        if file_metadata['data_file_extension'] in [".byml", ".bgyml"]:
            data = byml.byml_tools.dump(f"{file_metadata['worktable_file_folder']}/{file_metadata['file_name']}{file_metadata['data_file_extension']}")
        files_data.append({
...
def set_ressource_data(files_metadata, files_data, modified_files_list, RSTB_DUMP):
    for file_metadata in files_metadata:
        for file_data in files_data:
            if file_data["file_name"] == file_metadata["file_name"] and file_data["ressource_type"] == file_metadata["worktable_file_folder"] and file_metadata['data_file_extension'] in [".byml", ".bgyml"]:
                byml.byml_tools.restore(f"{file_metadata['worktable_file_folder']}/{file_metadata['file_name']}{file_metadata['data_file_extension']}",file_data["file_data"])
        with contextlib.suppress(Exception):
            for name, RSTB in RSTB_DUMP.items():
                RSTB.delete_entry(os.path.normpath(f"{file_metadata['romfs_file_folder']}/{file_metadata['file_name']}{file_metadata['data_file_extension']}").replace(os.sep, '/').replace("romfs/", ""))                 
        if f"{file_metadata['worktable_file_folder']}/{file_metadata['file_name']}" in modified_files_list:
            ...
            if file_metadata["compressed"] == True:
                if file_metadata["compression_metadata"]["compression_extension"] == ".zs":
                    zstandard.zstandard_tools.compress(f"{file_metadata['worktable_file_folder']}/{file_metadata['file_name']}{file_metadata['data_file_extension']}", f"{file_metadata['output_file_folder']}/{file_metadata['file_name']}{file_metadata['data_file_extension']}{file_metadata['compression_metadata']['compression_extension']}")
            else:
